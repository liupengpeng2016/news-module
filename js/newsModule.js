...
function NewsModule (params) {
  ...
  this.adAdsenseid = params.adAdsenseid
  ...
  if (!params.adId || !params.newsAdsenseid || !params.adAdsenseid) {
    throw new Error('adsenseid is required')
    return
  }
  this.state = {
    activeIndex: 0,
...
NewsModule.prototype = {
  ...
  createNewsTop: function () {
    ...
    // 360橱窗
    setTimeout(function () {
      var obj = {"w":280,"showid":"OBHnX7","rshowid":"OBHnX7","inject":"inlay","layout":"magicCubeScene","style":"magicCube","row":"2","line":"2"}
  ...
  baiduAdTemplate: function (num) {
    ...
    return '<iframe frameborder="0" width="100%" id="' + iframeId + '" scrolling="no" src="' + this.baiduAdUrl + '#' + iframeId + '"/>'
  },
  get360Ad: (function () {
    ...
    for (var i = 0; i < newsData.length; i++) {
      ...
      if ((i === 0 || i % 3 === 0) && this.showAd) {
        ...
        switch (type) {
          ...
          case '360': ad = this._360AdContainer(this.adId); adNum360++;break
        }
        this.adId++
      }
      // news
  ...
  init: function () {
    ...
    function clickTabHandle () {
     ...
     function scrollLoadHandle () {
       ...
       var clientHeight = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight
       if (Math.ceil(scrollTop + clientHeight) >= docHeight - newsModule.instance) {
